...
class KceClient(AbstractClient):
    ...
    def UpdateClusterDelProtection(self, request):
        """UpdateClusterDelProtection
        :param request: Request instance for UpdateClusterDelProtection.
        :type request: :class:`ksyun.client.kce.v20231115.models.UpdateClusterDelProtectionRequest`
        """
        try:
            params = request._serialize()
            body = self.call_judge("UpdateClusterDelProtection", params, "application/x-www-form-urlencoded")
            response = json.loads(body)
            if "Error" not in response:
                return body
            else:
