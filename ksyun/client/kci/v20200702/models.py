...
当配置HostPID=true时，容器不创建独立的PID命名空间，而是直接共享宿主机的全部进程列表，容器内部可以查看、操作宿主机上的所有进程（需配合相应权限）；当配置为false（默认值）时，容器会拥有独立的PID命名空间，仅能看到自身内部的进程，与宿主机进程完全隔离。
    ...
    def _deserialize(self, params):
        ...
        if params.get("HostPID"):
            self.HostPID = params.get("HostPID")
        if params.get("InitContainer"):
            self.InitContainer = params.get("InitContainer")
...
支持：asc, desc
    ...
    def _deserialize(self, params):
        ...
        if params.get("SortDirection"):
            self.SortDirection = params.get("SortDirection")
